...
<style>
    ...
    /* サイズ設定 */
    /* 左端(-3) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(1) div[data-testid="stMarkdownContainer"] + div { width: 36px !important; height: 36px !important; }
    /* 左中(-2) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(2) div[data-testid="stMarkdownContainer"] + div { width: 28px !important; height: 28px !important; }
    /* 左小(-1) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(3) div[data-testid="stMarkdownContainer"] + div { width: 20px !important; height: 20px !important; }
    /* 中央(0) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(4) div[data-testid="stMarkdownContainer"] + div { width: 16px !important; height: 16px !important; }
    /* 右小(+1) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(5) div[data-testid="stMarkdownContainer"] + div { width: 20px !important; height: 20px !important; }
    /* 右中(+2) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(6) div[data-testid="stMarkdownContainer"] + div { width: 28px !important; height: 28px !important; }
    /* 右端(+3) */
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(7) div[data-testid="stMarkdownContainer"] + div { width: 36px !important; height: 36px !important; }
    ...
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(3):hover div[data-testid="stMarkdownContainer"] + div { border-color: #E57373 !important; }
    ...
    div[data-testid="stForm"] div[role="radiogroup"] label:has(input:checked):nth-of-type(3) div[data-testid="stMarkdownContainer"] + div { background-color: #F44336 !important; border-color: #F44336 !important; transform: scale(1.1); }
    ...
    div[data-testid="stForm"] div[role="radiogroup"] label:nth-of-type(7):hover div[data-testid="stMarkdownContainer"] + div { border-color: #81C784 !important; }
    ...
    div[data-testid="stForm"] div[role="radiogroup"] label:has(input:checked):nth-of-type(7) div[data-testid="stMarkdownContainer"] + div { background-color: #4CAF50 !important; border-color: #4CAF50 !important; transform: scale(1.1); }
    ...
    /* ボタンを中央配置（Cloud対応） */
    div[data-testid="stForm"] > div[data-testid="stHorizontalBlock"]:last-child {
        display: flex !important;
        justify-content: center !important;
        align-items: center !important;
        gap: 15px !important;
    ...
    .stButton button { width: 100%; font-weight: bold; padding: 12px 0; border-radius: 20px; font-size: 1.1rem; }
    .agree-label { text-align: left; color: #4CAF50; font-weight: bold; font-size: 1.1rem; padding-top: 5px; }
    .disagree-label { text-align: right; color: #F44336; font-weight: bold; font-size: 1.1rem; padding-top: 5px; }
    ...
    @media (max-width: 640px) { div[data-testid="stForm"] div[role="radiogroup"] { gap: 8px; } .agree-label, .disagree-label { font-size: 0.9rem; } }
</style>
...
def get_type_info(result_type):
    ...
    type_map = {
        "INTJ": {"group": "分析家 (Architect)", "color": color_nt, "image": "intj.png"},
        "INTP": {"group": "分析家 (Logician)", "color": color_nt, "image": "intp.png"},
        "ENTJ": {"group": "分析家 (Commander)", "color": color_nt, "image": "entj.png"},
        "ENTP": {"group": "分析家 (Debater)", "color": color_nt, "image": "entp.png"},
        "INFJ": {"group": "外交官 (Advocate)", "color": color_nf, "image": "infj.png"},
        "INFP": {"group": "外交官 (Mediator)", "color": color_nf, "image": "infp.png"},
        "ENFJ": {"group": "外交官 (Protagonist)", "color": color_nf, "image": "enfj.png"},
        "ENFP": {"group": "外交官 (Campaigner)", "color": color_nf, "image": "enfp.png"},
        "ISTJ": {"group": "番人 (Logistician)", "color": color_sj, "image": "istj.png"},
        "ISFJ": {"group": "番人 (Defender)", "color": color_sj, "image": "isfj.png"},
        "ESTJ": {"group": "番人 (Executive)", "color": color_sj, "image": "estj.png"},
        "ESFJ": {"group": "番人 (Consul)", "color": color_sj, "image": "esfj.png"},
        "ISTP": {"group": "探検家 (Virtuoso)", "color": color_sp, "image": "istp.png"},
        "ISFP": {"group": "探検家 (Adventurer)", "color": color_sp, "image": "isfp.png"},
        "ESTP": {"group": "探検家 (Entrepreneur)", "color": color_sp, "image": "estp.png"},
        "ESFP": {"group": "探検家 (Entertainer)", "color": color_sp, "image": "esfp.png"},
    }
...
def main():
    st.markdown("""<script>document.addEventListener('DOMContentLoaded', function() {setTimeout(function() { window.scrollTo(0, 0); }, 100);});</script>""", unsafe_allow_html=True)
    ...
    if st.session_state.page == 0:
        ...
        st.markdown("### 👤 基本情報")
        st.radio("性別（任意）", ["男性", "女性", "その他", "回答しない"], horizontal=True, key="gender_input", label_visibility="collapsed")
    ...
    with st.form(key=f"form_page_{st.session_state.page}"):
        ...
        is_first = (st.session_state.page == 0)
        is_last = (st.session_state.page == TOTAL_PAGES - 1)
        
        if is_first:
            # 初回は次へだけ
            cols = st.columns([2, 2, 2])
            with cols[1]:
                st.form_submit_button("次へ ＞", type="primary", use_container_width=True, on_click=save_and_next)
        else:
            # 2つのボタンを横並び
            cols = st.columns([2, 1, 1, 2])
            with cols[1]:
                st.form_submit_button("＜ 前へ", use_container_width=True, on_click=save_and_prev)
            with cols[2]:
                if is_last:
                    st.form_submit_button("診断結果を見る ＞", type="primary", use_container_width=True, on_click=save_and_finish)
                else:
                    st.form_submit_button("次へ ＞", type="primary", use_container_width=True, on_click=save_and_next)
