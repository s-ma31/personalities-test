...
# コールバック関数：値を保存してページ移動
def go_next():
    # 現在のページの質問範囲
    start = st.session_state.page * QUESTIONS_PER_PAGE
    end = start + QUESTIONS_PER_PAGE
    # ウィジェットのキー(radio_{id})から値を回収してanswersに保存
...
def main():
    ...
    with st.form(key=f"form_page_{st.session_state.page}"):
        ...
        for q in current_questions:
            ...
            with c2:
                # session_state.answers から保存済みの値を取得（デフォルト0）
                current_val = st.session_state.answers.get(q['id'], 0)
                
                # キーを個別に設定
                # 【重要】ループ内では保存処理（answersへの代入）を行わず、コールバックに任せる
                st.radio(
                    f"質問 {q['id']}",
                    options,
        ...
        # ボタンエリアのカラム比率を調整して中央に寄せる
        b_col1, b_col2, b_col3 = st.columns([3, 2, 3])
        
        # 最初のページ（次へボタン1つ）
        if is_first_page:
            with b_col2:
                st.form_submit_button("次へ ＞", type="primary", use_container_width=True, on_click=go_next)

        # 中間のページ以降（ボタン2つ）
        else:
            # 2つのボタンを中央に配置するためのカラム構成
            b_col_prev, b_col_next = st.columns([1, 1]) 
            # もっと中央に寄せたい場合、外側に空白カラムを作る
            col_outer_L, col_btn_L, col_btn_R, col_outer_R = st.columns([2, 2, 2, 2])
            
            with col_btn_L:
                st.form_submit_button("＜ 前へ", use_container_width=True, on_click=go_prev)
            
            with col_btn_R:
                if is_last_page:
                    st.form_submit_button("診断結果を見る ＞", type="primary", use_container_width=True, on_click=go_finish)
                else:
                    st.form_submit_button("次へ ＞", type="primary", use_container_width=True, on_click=go_next)
